...
import kotlinx.android.synthetic.main.activity_bianli__xh.*

class Bianli_XHActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_bianli__xh)
        ...
        btn_repeat_subscript.setOnClickListener {
            ...
            for (i in poemArray.indices) {
                poem = if (i % 2 == 0) "$poem${poemArray[i]}，\n" else "$poem${poemArray[i]}。\n"
            }
            tv_poem_content.text = poem
        }
        ...
        btn_repeat_continue.setOnClickListener {
            ...
            while (pos<=poem2Array.size){
                ...
                if(poem2Array[pos].isNullOrBlank())
                    continue
                poem=if(count%2==0) "$poem${poem2Array[pos]}，\n" else "$poem${poem2Array[pos]}。\n"
                count++
                //合法行数达到四行，则结束循环
                if(count==4)
                    break
